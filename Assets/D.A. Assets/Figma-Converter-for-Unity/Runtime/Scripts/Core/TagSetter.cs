...
namespace DA_Assets.FCU
{
    ...
    public class TagSetter : MonoBehaviourLinkerRuntime<FigmaConverterUnity>
    {
        ...
        private bool CanBeSingleImage(FObject fobject)
        {
            if (fobject.Children.IsEmpty())
            {
                Debug.Log($"Reason: 1 - Object has no children. | {fobject.Data.NameHierarchy}");
                return false;
            }
        ...
        private bool CanBeInsideSingleImage(FObject fobject, FObject parent)
        {
            ...
            {
                reason = 2;
                Debug.Log($"Reason: {reason} - ForceContainer is true. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}");
                return false;
            }
            ...
            {
                reason = 3;
                Debug.Log($"Reason: {reason} - ForceImage is true. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}");
                return false;
            }
            ...
            foreach (FcuTag fcuTag in fobject.Data.Tags)
            {
                ...
                {
                    reason = 4;
                    Debug.Log($"Reason: {reason} - Tag {fcuTag} cannot be inside a single image. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}");
                    return false;
                }
            }
